...
// Реализация интерфейсов raft.LogStore и raft.StableStore
...
func (r *RaftStableStore) Set(key []byte, val []byte) error {
	r.store[string(key)] = val
	return nil
}

func (r *RaftStableStore) Get(key []byte) ([]byte, error) {
	return r.store[string(key)], nil
}
...
func main() {
	...
	// Create an hclog.Logger instance
	logger := hclog.New(&hclog.LoggerOptions{
		Name:   "raft",
		Output: os.Stderr,
		Level:  hclog.LevelFromString("DEBUG"), // Adjust log level as needed
	})
